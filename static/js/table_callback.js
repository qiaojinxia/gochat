...
function changeCell(changes, source) {
    if ( source !== "edit" || !hot)
        return;
    let data = hot.getData();
    let header = GetSheetHeader(curSelectTablesName);
    let workbook = XLSX.utils.book_new();

    const sheet = XLSX.utils.aoa_to_sheet( [header, ...data]);
    XLSX.utils.book_append_sheet(workbook, sheet, 'Sheet1');
    let saveSheet = XLSX.write(workbook, {bookType:'xlsx', type:'array'});
    SaveSheet(curSelectTablesName,saveSheet);
    needUploadFile = new File([saveSheet], curSelectTablesName, { type: 'application/octet-stream' });
    sendMsg(true);
    needUploadFile = undefined;
}